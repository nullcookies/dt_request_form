...
class RequestForm extends FormBase
{
    ...
    public function __construct()
    {
        ...
        //Проверяем сущесвует ли такой rule
        $query = \Drupal::entityQuery('rules_reaction_rule')->allRevisions()->execute(); //TODO
        if (!array_key_exists("creating_reviews_for_your_organization", $query)) {
            $this->is_rule = false;
        }
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        if ($this->is_rule) {
            ...
            $form['field_message'] = [
                '#type' => 'item',
                '#title' => t('New review'),
                '#markup' => t('You are about to create a new review'),
                '#weight' => -6,
            ];

            $form['review'] = [
                '#type' => 'details',
                '#title' => ('Review for organization'),
                '#collapsible' => false,
                '#collapsed' => false,
                '#open' => TRUE,
            ];
    ...
    public function validateForm(array &$form, FormStateInterface $form_state)
    {
        ...
        if ($this->is_rule) {
            ...
            if (empty($form_state->getValue('title'))) {
                ...
                $form_state->setErrorByName('title', $this->t('Field must not be empty.'));
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        if ($this->is_rule) {
            ...
            $params['message'] = 'Thank you! Review saving';
            $node = Node::create([
                'type' => 'Review',
                'title' => $form_state->getValue('title'),
                'body' => $form_state->getValue('body'),
              /* 'field_shop' => $form_state->getValue('field_shop'),*/
